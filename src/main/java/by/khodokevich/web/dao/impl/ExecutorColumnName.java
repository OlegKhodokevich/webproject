package by.khodokevich.web.dao.impl;

public class ExecutorColumnName {
    protected static final String ID_USER = "IdUser";
    protected static final String FIRSTNAME = "FirstName";
    protected static final String LASTNAME = "LastName";
    protected static final String PASSWORD = "EncodedPassword";
    protected static final String E_MAIL = "EMail";
    protected static final String PHONE = "Phone";
    protected static final String REGION = "Region";
    protected static final String CITY = "City";
    protected static final String STATUS = "UserStatus";
    protected static final String ROLE_STATUS = "UserRole";
    protected static final String PERSONAL_FOTO = "PersonalFoto";
    protected static final String UNP = "UNP";
    protected static final String DESCRIPTION = "DescriptionExecutor";
    protected static final String AVERAGE_MARK = "AverageMark";
    protected static final String NUMBER_COMPLETION_CONTRACTS = "NumberCompletionContracts";
    protected static final String NUMBER_CONTRACTS_PROGRESS = "NumberContractsInProgress";
    protected static final String SPECIALIZATION = "Specialization";
    protected static final String COST = "Cost";
    protected static final String MEASURE = "Measure";

    private ExecutorColumnName() {
    }
}
